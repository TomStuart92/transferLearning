...
class ViewController: UIViewController, UIImagePickerControllerDelegate {
    ...
    func setupVision() {
        guard let visionModel = try? VNCoreMLModel(for: locksmodel.model) else {
        ...
        let classificationRequest = VNCoreMLRequest(model: visionModel) { (request: VNRequest, error: Error?) in
            ...
            if let feature = observations[0] as? VNCoreMLFeatureValueObservation {
                let labels = ["mortice_deadlock", "multipoint_lock", "automatic_deadlatch"]
                
                
                let classifications = feature.featureValue.multiArrayValue as! MLMultiArray
                print(classifications)

                DispatchQueue.main.async {
                    self.predictLabel.text = "\(labels[0]): \(classifications[0] as! Float),\n \(labels[1]): \(classifications[1] as! Float),\n \(labels[2]): \(classifications[2] as! Float)"
                }
            }
